		  ...
		  where: {username: req.body.username, active: true}
			...
			if(userData){
				bcrypt.compare(req.body.password, userData.dataValues.password, function(err, match) {
					...
					{
						var token = jwt.sign({ fname: userData.dataValues.firstname, lname: userData.dataValues.lastname, username: userData.dataValues.username, email: userData.dataValues.email, permission: userData.dataValues.permission, active: userData.dataValues.active }, secret, {expiresIn: '24h'});
						//console.log(token);
						res.json({success:true, message:'User Authenticated!', token: token});
					}
				});
		...
		UserTable.findAll().then(function(users) {
			if(req.decoded.permission == 'admin' || req.decoded.permission == 'moderator'){
				if(!users){
					...
					res.json({success: true, users: users, permission: req.decoded.permission});
				}
			}else{
